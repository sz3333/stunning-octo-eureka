...
class MultiFemburnTGMod(loader.Module):
    """Отправка артов e621 по общим тегам (все теги должны совпасть)"""
    ...
    async def multifemburncmd(self, message):
        ...
        self.running = True
        await message.edit(f"🧦 Отправляю {count} артов с тегами: {', '.join(tags)}")
    ...
    async def _send_posts(self, message, tags, count):
        ...
        sent = 0
        # объединяем все теги в один запрос
        tag_query = "+".join(tags)
