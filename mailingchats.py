...
# meta name: MailingFolders
# meta desc: |
#   📂 MailingFolders — рассылка по папкам Telegram
#   🔹 Добавляй/удаляй папки по ID
#   🔹 Сообщения шлются во все чаты внутри выбранных папок
#   🔹 Сейф-режим и лимиты от бана
...
@loader.tds
class MailingFoldersMod(loader.Module):
    """Mailing по папкам Telegram"""
    
    strings = {
        "name": "MailingFolders",
        "loading": (
            "📂 MailingFolders загружен!\n\n"
            "🔹 Команды:\n"
            ".msgo <время> <интервал> <текст> — запуск рассылки\n"
            ".msstop — остановка рассылки\n"
            ".msfadd <id> — добавить папку\n"
            ".msfdel <id> — удалить папку\n"
            ".msflist — список папок\n"
            ".msstats — статистика\n"
            ".msmode — включить/выключить защиту от бана\n\n"
            "ℹ️ ID папок можно узнать через .folders"
        ),
        ...
        "no_folders": "❌ Нет папок для рассылки. Добавьте их через .msfadd <id>",
    ...
    async def _get_chats_from_folders(self):
        ...
        for f in res.filters:
            ...
            if f.id in self.config["ms_folders"]:
                ...
                for p in peers:
                    if isinstance(p, (PeerChannel, PeerChat, PeerUser)):
                        chats.append(p)
        return chats
    ...
    async def msgo(self, message):
        ...
        if not chats:
            await utils.answer(message, self.strings["no_folders"])
            return
